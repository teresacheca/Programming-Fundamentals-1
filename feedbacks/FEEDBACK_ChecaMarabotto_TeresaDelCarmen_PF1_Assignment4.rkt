...
;The structure line2 is a type. So, you must write:
;a Line2 is a structure...

;Also, instead of Maybe<Line>, you must write Maybe<Line2>.

;Color is a variable that does not change in your program. So, you do not need to put it in your AppState.
;You only need to put those variables in the AppState that can change during the execution of the program.

;There is only one test in your code. You must always write tests for all of the functions in your program.

;In cancel-line, you do not need to check if the line is false or not. You can simply set it to false.

;The signature of handle-mouse is AppState Number Number MouseEvent -> AppState.

;The signature of handle-key is AppState KeyEvent -> AppState.
